...
class CMIL(object):
    def __init__(self, obs_dim, action_dim, action_range, batch_size, args):
        ...
        # self.fixed_std = agent_cfg.dynamics_cfg.fixed_std
        self.unc_penalty = agent_cfg.unc_penalty
    ...
    def infer_r(self, state, action):
        ...
        self.dy_std = std = self.dynamics.compute_mean_std(obs_action, use_elites=True)
        return self.discriminator(state, action) - self.unc_penalty * std
    ...
    def update_critic(self, policy_batch, expert_batch):
        ...
        loss_dict['unc_std'] = self.dy_std.mean().item()
        return loss_dict
