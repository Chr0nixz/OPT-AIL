...
class MBPO(object):
    ...
    def update(self, policy_buffer, generate_buffer, expert_buffer, logger, step, epoch):
        ...
        policy_batch = policy_buffer.get_samples(policy_batch_size, self.device)
        generate_batch = generate_buffer.get_samples(generate_batch_size, self.device)
        batch = get_concat_samples(policy_batch, generate_batch)[:5]

        losses = self.update_critic(batch)
        if self.actor and step % self.actor_update_frequency == 0:
            if self.args.num_actor_updates:
                for i in range(self.args.num_actor_updates):
                    actor_alpha_losses = self.update_actor_and_alpha(batch[0])
    ...
    def learn_dynamics(self, inputs, targets, batch_size):
        self.dynamics.train()
    ...
    def load(self, path, suffix=""):
        ...
        print('Loading models from {}, {}'.format(actor_path, critic_path))
